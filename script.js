// --- TURBO MODE SINGLE TOGGLE ---
    ...
    // Sub-abas de medalhas (ATUALIZADO COM MINISTÉRIO)
  ...
  // =========================================================
    // === LÓGICA DE LIDERANÇA (MACRO & DATAS) ===
    // =========================================================
    ...
    async function loadLeadershipData() {
        ...
        try {
            ...
            // 2. Carrega Licenças e Retornos via MACRO
            ...
            leaders.forEach(([nickKey, data]) => {
                // nickKey é a chave em minúsculo, data.displayNick é o nome original
    ...
    function parseLeavesFromJSON(rows) {
        ...
        // Formato esperado do JSON (v2): 
        // Col A (0): Data | Col B (1): Tipo | Col C (2): Nick | Col D (3): Dias
    ...
    function parseDateCustom(str) {
        ...
        // 1. Tenta pegar dd/mm/yyyy no início da string
        ...
        // 2. Fallback para formato ISO (yyyy-mm-dd)
        ...
        // 3. Última tentativa: Date nativo
    ...
    async function openLeaderDetails(nick, role, month, year) {
        ...
        // Cria datas limpas (meio-dia) para evitar problemas de fuso horário
        ...
        if (leaves.length > 0) {
            ...
            // 1. Data que a ausência REALMENTE começa (Dia seguinte à postagem)
            const effectiveAbsenceStart = new Date(leaveDate);
            effectiveAbsenceStart.setDate(leaveDate.getDate() + 1);

            // 2. Data limite da licença (Data Licença + Dias Solicitados)
            ...
            // --- ATUALIZAÇÃO VISUAL BASEADA NO CÁLCULO ---
        ...
        const confirmBtn = document.getElementById('modal-confirm');
        const cancelBtn = document.getElementById('modal-cancel');
        // Usa o nick original para exibir, mas usa minúsculo para APIs se necessário (geralmente habbo API aceita ambos)
    ...
    async function fetchSmart(targetUrl, isGViz = false) {
        ...
        // Lista de proxies em ordem de prioridade/estabilidade
        const proxies = [
            (url) => `https://corsproxy.io/?${encodeURIComponent(url)}`,
            (url) => `https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`,
            (url) => `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(url)}` // Fallback robusto
        ];
        ...
        for (const proxyGen of proxies) {
            try {
                ...
                // Validação básica se retornou algo útil
    ...
    // =========================================================
    // === SISTEMA DE MP AUTOMÁTICA (INTEGRAÇÃO) ===
    // =========================================================
    
    // 1. Gera o template (Async para suportar fetch externo)
    ...
    function updateMedalLink() {
        ...
        const date = document.getElementById('med-date').value;
        // Botão fica habilitado se houver dados, mas o clique vai validar o ativo
        btn.disabled = !(responsible && result && date);
    }

    // --- NOVA LÓGICA DE FILTRAGEM RESTRITIVA ---
    function getActiveNicksFromFilter() {
        const nicks = document.getElementById('med-result').innerText;
        const activeInputText = document.getElementById('active-list-input').value;
        
        // Validação Rigorosa: Se não tiver lista de ativos, não retorna nada
        ...
        // Filtra apenas quem está no set de ativos
    ...
    // Botão Postar Medalhas
    function openMedalLink() {
        const activeNicks = getActiveNicksFromFilter();
        if (activeNicks === null) return; // Erro de input vazio
        if (activeNicks.length === 0) return; // Nenhum ativo encontrado
    ...
    // Botão Disparar MP Automática (Novo)
    async function openPMLink() {
        ...
        // 2. Confirmação com Modal Personalizado
    ...
    // --- NOVA FUNÇÃO PARA CARREGAR DADOS DE MEMBROS (EM TSV PARA PRECISÃO) ---
    async function fetchMembersData() {
        if(Object.keys(membersCache).length > 0 && !forceBypassCache) return;
        
        try {
            // USANDO TSV CONFORME PEDIDO
            ...
            // Define data atual "meio-dia" para evitar problemas de fuso
            const today = new Date();
            ...
            rows.forEach(row => {
                ...
                const originalNick = cleanCell(row[1]); // Mantém Maiúsculas
                const nick = originalNick.toLowerCase(); // Chave de busca em minúsculo
                const dataRaw = cleanCell(row[3]);
                
                if(!nick || !cargo) return;

                let promoDate = null;
                // Parser de data flexível (aceita dd/mm/yyyy e yyyy-mm-dd)
                if(dataRaw) {
                    if(dataRaw.includes('/')) {
                        ...
                        const parts = dataRaw.split('-');
                        // yyyy-mm-dd (padrão sheets export)
                ...
                if(promoDate && !isNaN(promoDate)) {
                    promoDate.setHours(12, 0, 0, 0); // Normaliza
                    const diffTime = Math.abs(today - promoDate);
                    // Arredonda para cima para ser inclusivo
                ...
                membersCache[nick] = {
                    ...
                    diffDays: diffDays,
                    rawDate: dataRaw,
                    displayNick: originalNick // NOVO CAMPO PARA EXIBIÇÃO
                };
            });
        } catch (e) {
    ...
    function generateBBCodeString(mode = "post") {
        ...
        if(mode === 'highlight') {
             return `[center][b][size=18]⭐ DESTAQUES DA SEMANA ⭐[/size][/b][/center]\n\n[center]${blockDestaques}[/center]`;
        }
    ...
    // Função Postar Destaques (Professor apenas, T=5)
    async function postHighlights() {
        ...
        // 1. Obter Tokens do Tópico 5
        try {
            await fetchTopicTokens('5'); // Força pegar tokens do tópico 5
        } catch(e) {
            showToast("Erro ao conectar com tópico de destaques (t=5).", "error");
